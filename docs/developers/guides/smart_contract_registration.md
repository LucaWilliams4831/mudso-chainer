...
fcod tx revenue register $CONTRACT $NONCE $WITHDRAWER \
...
```bash
# Check revenues
fcod q revenue contract $CONTRACT
```
...
fcod tx revenue update $CONTRACT $WITHDRAWER \
...
fcod tx revenue cancel $CONTRACT \
--gas=700000 --gas-prices=10000pose \
--from=mm
```
