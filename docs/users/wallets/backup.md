...
```bash
$ fcod keys add dev0-restored --recover
...
```bash
fcod keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ fcod keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ fcod keys unsafe-import-eth-key dev0-imported ./dev0.export
