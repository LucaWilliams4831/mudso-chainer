...
```bash
fcod init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .fcod/config folder

```bash
mv $HOME/genesis.json ~/.fcod/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.fcod/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.fcod/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.fcod/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.fcod/config/config.toml
```
...
```bash
echo "[Unit]
Description=fcod Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which fcod) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/fcod.service; sudo mv $HOME/fcod.service /etc/systemd/system/
```

```bash
sudo systemctl enable fcod.service && sudo systemctl daemon-reload
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.fcod/config/config.toml
```
