...
Find below a list of `fcod` commands added with the `x/vesting` module. You can obtain the full list by using the `fcod -h` command.
...
```go
fcod add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
fcod query vesting balances ADDRESS [flags]
```
...
```go
fcod tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
fcod tx vesting clawback ADDRESS [flags]
```
...
```go
fcod tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
